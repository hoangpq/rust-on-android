...
Local<Value> JavaFunctionWrapper::NewInstance(JavaType *type, Isolate *isolate,
                                              jobject jinst,
...
void JavaFunctionWrapper::Call(const FunctionCallbackInfo<Value> &args) {
  ...
  for (JFunc &func : wrapper->_type->funcList) {
    if (func.argumentCount == argumentCount &&
      ...
      jmethodID methodId =
          env->GetMethodID(wrapper->_type->GetJavaClass(),
                           func.methodName.c_str(), func.sig.c_str());

      if (func.argumentCount == 0) {
        jint size =
            env->CallIntMethod(wrapper->_type->GetJavaInstance(), methodId);
        jsValue = Number::New(isolate, size);
      } else {
        jobject jValue = V8ToJava(args[0]);
        env->CallBooleanMethod(wrapper->_type->GetJavaInstance(), methodId,
                               jValue);
